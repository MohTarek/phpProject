...
class Database {
    ...
    private $password="";
    private $db_name="bank";//your database name 
   ...
   //Clean data that is used in SQL queries.
     function clean($str) {
		$str = trim($str); // remove spaces
                $str = stripslashes($str); //remove slashes and '
                $str = strip_tags($str);// remove tages HTML
		$str= mysql_real_escape_string($str);//remove sql query 
                return $str;
...
public function update($table, $data, $where,$x){
    $q="UPDATE `$table` SET ";
